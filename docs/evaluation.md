...
> **能力边界**
>
> `builtin:smoke-v1` 与 `builtin:temporal-v1` 都是项目自有的 synthetic 契约集。它们不是 LongMemEval、LoCoMo 或其他论文 benchmark，不衡量完整 Agent 的最终答案质量，也不能证明 SOTA、生产质量或跨数据集泛化能力。
...
`temporal-v1` 使用 schema v2 的显式 `run_at` 时间线推进评测专用时钟，并额外覆盖：

- 迟到修正在被系统记录前后选择不同 Revision，且精确覆盖 `recorded_at == known_at` 边界；
- 已知但未来生效的修订在 `valid_at` 到达边界前不可见、到达后可见；
- Outcome 必须引用先前 RecallTrace，重复 Outcome 保持幂等，Utility 在其知识时间前后分别为先验值与更新值；
- 早于首条 Revision 的有效时间拒答、带双时间的 Scope 隔离，以及未来 `known_at` 的预期 `DomainError`；
- 只有允许清单中的领域拒绝可以被标注为预期错误，运行时异常不能被数据标签包装成通过。

schema v1 不接受 `run_at` 或 Outcome case；schema v2 要求每个 case 都有带时区且非递减的 `run_at`。该字段只控制隔离评测时钟，不是 API 字段，也不会让线上调用者控制服务器 `recorded_at`。

两个数据集都不替代单元测试、PostgreSQL 集成测试、浏览器 E2E、安全测试或真实业务的授权评测集。旧 Outcome 的迁移时间仍只能由 PostgreSQL upgrade/downgrade 测试验证为 best-effort 近似，`temporal-v1` 不会把迁移前不存在的摄入时间伪造成精确事实。
...
| 写入/修正/Outcome case | 执行数量及逐 case 结果 | 幂等、Utility 或归因断言失败同时计入 execution failure |
...
两个内置数据集的公开命令都要求 Recall、MRR、更新准确率和拒答准确率为 `1.0`，同时要求 forbidden hit 和 execution failure 为 `0`；Outcome、Utility 与预期拒绝断言通过 execution failure 硬门禁执行。显式阈值参数适合受控实验，但不得用降低阈值的方式把安全回归包装成通过。
...
- 当前提交通过 `builtin:smoke-v1` 的 retrieval/invariant 契约和 `builtin:temporal-v1` 的双时间/Outcome 契约；
- 在对应合成数据快照上，Recall@k、MRR、双时间状态选择、历史 Utility、拒答和硬门禁结果可复现。
...
- 双时间契约代表完整历史策略 replay，或证明迁移前 Outcome 的 system-time 精确；
