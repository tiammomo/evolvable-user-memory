...
- 把 `tenant_id` 和 `subject_id` 暴露为开发合同，不提供可信身份认证或授权；
...
威胁、信任边界和生产安全验收门槛见[威胁模型](docs/threat-model.md)；隐私生命周期的目标工作流和验收标准见[隐私生命周期设计](docs/privacy-lifecycle.md)。这两份文档都是设计基线，不表示能力已经实现。进一步的生产缺口和顺序见 [ROADMAP.md](ROADMAP.md) 与[部署指南](docs/deployment.md)。
