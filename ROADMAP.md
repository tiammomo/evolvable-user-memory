...
- 从不可变 RecallTrace 生成字符预算内规范 JSON 的确定性上下文压缩，支持排序抽取、精确去重、逐片段 revision 归因和可复现 SHA-256；
...
这一阶段仍不具备完整 IdP 角色治理、token 撤销/临时授权、数据库 RLS、自动保留/备份治理或完整生产运维能力；在线删除证明已经实现，内存模式继续作为显式开发选项。

当前双时间能力重建的是 Revision 与 Outcome 的历史可见状态，评分仍使用请求执行时的不可变 `StrategySnapshot`。完整历史策略、投影版本和运行环境 replay 尚未实现；旧 Outcome 的系统时间也是迁移时的 best-effort 近似，不能升级为精确历史审计声明。
...
- 在已实现的 ProcessingGrant、抑制、在线删除和投影清理证明上补自动保留、备份/导出副本和恢复屏障演练；
...
- 在现有 Milvus embedding 和 Trace 上下文压缩之外增加关键词、图和模型驱动摘要投影，并验证跨模型重建/切换；
